...
public class TransController {
    ...
    // 거래 내역 조회
    @GetMapping
    public List<TransDto> readTransaction(@RequestParam(name = "accountIdx") int accountIdx){
        return transService.readTrans(accountIdx);
    }
    // 거래 (입출금 및 송금)
    @PostMapping
    public TransDto createTransaction(@RequestBody TransDto transDto){
        return transService.createTransaction(transDto);
    }
    // 거래 취소
    @DeleteMapping
    public TransDto cancelTransaction(@RequestBody TransDto transDto){
